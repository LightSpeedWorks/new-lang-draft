...
BlockSyntax.prototype.toString = function toString() {
  ...
  for (var i = 0, n = this.syntaxes.length; i < n; ++i) {
    ...
    if (syntax instanceof StatementSyntax)
      s += syntax + '';
    else if (syntax instanceof ExpressionSyntax)
      s += syntax + '; ';
    else
      s += '/*eh?{*/' + syntax + '/*}?*/ ';
  }
...
function ExpressionStatementSyntax(syntax) {
  StatementSyntax.call(this, 200);
  this.syntax = syntax;
}
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
function VarSyntax(syntax) {
  StatementSyntax.call(this, 190);
  this.syntax = syntax;
}
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
VarSyntax.prototype.run = function run(ctx) {
  var varSymExprs = this.syntax
  for (var i = 0, n = varSymExprs.length; i < n; ++i) {
    var symExpr = varSymExprs[i];
    if (symExpr.length === 2)
...
AssignSyntax.prototype.run = function run(ctx) {
  return this.syntax1.locate(ctx).setValue(this.syntax2.run(ctx));
};
//----------------------------------------------------------------------
...
AssignAddSyntax.prototype.run = function run(ctx) {
  ...
  return loc.setValue(loc.getValue() + this.syntax2.run(ctx));
};
//----------------------------------------------------------------------
...
function LambdaSyntax(op, syntax1, syntax2) {
  BinSyntax.call(this, 170, op, syntax1, syntax2);
}
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
function ElvisSyntax(op, syntax1, syntax2) {
...
ElvisSyntax.prototype.run = function run(ctx) {
  ...
  // Elvis operator
  return val != null ? val : this.syntax2.run(ctx);
};
//----------------------------------------------------------------------
...
Cond3Syntax.prototype.toString = function toString() {
  ...
  var expr2 = this.syntax2.toString();
  if (this.prio < this.syntax2.prio) expr2 = '(' + expr2 + ')';
  var expr3 = this.syntax3.toString();
